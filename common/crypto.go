...
import (
	...
	"github.com/qiaopengjun5162/VRFService/hsm"
)
...
// ParsePrivateKeyStr parses a private key string and returns an ECDSA private key.
//
// This function takes a private key string as input, removes the "0x" prefix if present,
// and converts it to an ECDSA private key.
//
// Parameters:
//   - privyKeyStr: A string representation of the private key, with or without the "0x" prefix.
//
// Returns:
//   - *ecdsa.PrivateKey: A pointer to the parsed ECDSA private key.
//   - error: An error if the parsing fails, or nil if successful.
func ParsePrivateKeyStr(privyKeyStr string) (*ecdsa.PrivateKey, error) {
	// 去掉私钥字符串的前缀 "0x"
	prefix := strings.TrimPrefix(privyKeyStr, "0x")
	// 将私钥字符串转换为 ecdsa 私钥
	return crypto.HexToECDSA(prefix)
}
