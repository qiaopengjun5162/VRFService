...
func newSendState() *tx_manager.SendState {
	return tx_manager.NewSendState(testSafeAbortNonceTooLowCount)
}

func processNSendErrors(sendState *tx_manager.SendState, err error, n int) {
	for i := 0; i < n; i++ {
