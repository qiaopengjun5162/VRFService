...
type testHarness struct {
	cfg       tx_manager.Config
	mgr       tx_manager.TxManager
	backend   *mockBackend
	gasPricer *gasPricer
}

func newTestHarnessWithConfig(cfg tx_manager.Config) *testHarness {
	backend := newMockBackend()
	mgr := tx_manager.NewSimpleTxManager(cfg, backend)
...
func configWithNumConfs(numConfirmations uint64) tx_manager.Config {
	return tx_manager.Config{
...
func (g *gasPricer) feesForEpoch(epoch int64) (*big.Int, *big.Int) {
	...
	epochGasFeeCap := tx_manager.CalcGasFeeCap(epochBaseFee, epochGasTipCap)
...
func TestWaitMinedReturnsReceiptOnFirstSuccess(t *testing.T) {
	...
	ctx := context.Background()
	receipt, err := tx_manager.WaitMined(ctx, h.backend, tx, 50*time.Millisecond, 1)
	require.Nil(t, err)
	require.NotNil(t, receipt)
...
func TestWaitMinedMultipleConfs(t *testing.T) {
	...
	receipt, err := tx_manager.WaitMined(ctxt, h.backend, tx, 50*time.Millisecond, numConfs)
	require.Equal(t, err, context.DeadlineExceeded)
	require.Nil(t, receipt)
...
func TestWaitMinedReturnsReceiptAfterFailure(t *testing.T) {
	...
	ctx := context.Background()
	receipt, err := tx_manager.WaitMined(ctx, &borkedBackend, tx, 50*time.Millisecond, 1)
	require.Nil(t, err)
	require.NotNil(t, receipt)
	require.Equal(t, receipt.TxHash, txHash)
}
